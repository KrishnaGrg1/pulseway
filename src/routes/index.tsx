...
function LatencyBar() {
  ...
  return (
    <div className="mt-5 rounded-lg border border-[var(--line)] bg-[var(--surface-0)] p-4">
      <p className="mb-3 font-mono text-[10px] tracking-widest text-[var(--text-3)] uppercase">
      ...
      <div className="mt-2 flex justify-between font-mono text-[9px] text-[var(--text-3)]">
        <span>−6 min</span>
        <span>now</span>
      </div>
    </div>
    ...
    <div className="flex min-h-screen flex-col bg-[var(--bg)]">
      ...
      <header className="border-b border-[var(--line)]">
        <div className="content-wrap flex items-center justify-between py-3 md:py-4">
          ...
          <nav className="hidden items-center gap-4 md:flex lg:gap-6">
            ...
            <button onClick={() => navigate({ to: '/login' })} className="btn btn-secondary btn-sm">
              Login
            </button>
      ...
      <section className="content-wrap py-12 md:py-20 lg:py-24">
        <div className="grid items-center gap-8 md:gap-12 lg:grid-cols-2">
          {/* LEFT */}
          <div>
            <p className="text-[10px] tracking-[0.25em] text-[var(--text-3)] uppercase md:text-xs">
              Uptime monitoring
            </p>

            <h1 className="mt-3 text-3xl leading-tight font-bold md:text-4xl lg:text-5xl">
              Know when your APIs
              <br />
            ...
            <p className="mt-4 max-w-lg text-sm text-[var(--text-2)] md:text-base">
              30-second checks, instant alerts, and real-time latency tracking built for developers.
            </p>

            <div className="mt-6 flex flex-col gap-2.5 sm:flex-row sm:gap-3">
              <button
                className="btn btn-primary md:btn-lg justify-center px-6 py-2.5 text-sm"
                onClick={() => navigate({ to: '/register' })}
              >
                Start free
              </button>
              ...
              <button
                className="btn btn-secondary md:btn-lg justify-center px-6 py-2.5 text-sm"
                onClick={() => navigate({ to: '/login' })}
              >
                Login
              </button>
            ...
            <div className="mt-6 flex items-center gap-3">
              ...
              <p className="text-xs text-[var(--text-2)]">
                <strong className="text-[var(--text-1)]">240+ developers</strong> using Pulseway
              </p>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-[var(--line)] bg-[var(--surface-0)] p-4 md:p-5">
            ...
            <div className="mt-3 grid grid-cols-3 gap-2 md:mt-4">
              {[
                ...
                <div key={s.label} className="rounded-lg border border-[var(--line)] p-3">
                  <div className="text-[10px] text-[var(--text-3)]">{s.label}</div>
                ...
                >
                  ...
                  <span
                    className={`rounded px-2 py-0.5 font-mono text-[10px] font-bold ${
                      m.up ? 'bg-[var(--lagoon)] text-black' : 'bg-[var(--danger)] text-white'
                    }`}
                  >
                    {m.up ? 'UP' : 'DOWN'}
                  </span>
                  ...
                  <div className="flex-1">
                    ...
                    <p className="m-0 font-mono text-[10px] text-[var(--text-3)]">{m.url}</p>
                  </div>
      ...
      <section id="features" className="content-wrap py-12 md:py-16 lg:py-20">
        ...
        <div className="grid gap-3 md:grid-cols-2 md:gap-4 lg:grid-cols-3">
          ...
          {FEATURES.slice(1).map((f) => (
            <div key={f.title} className="rounded-xl border border-[var(--line)] p-5">
      ...
      <section id="how-it-works" className="content-wrap py-12 md:py-16 lg:py-20">
        ...
        <div className="grid gap-3 md:grid-cols-3 md:gap-4">
          {HOW_IT_WORKS.map((s) => (
            <div key={s.step} className="rounded-xl border border-[var(--line)] p-5">
              <div className="font-mono text-sm text-[var(--lagoon)]">{s.step}</div>
      ...
      <section className="content-wrap py-12 md:py-16 lg:py-20">
        <div className="rounded-xl bg-gradient-to-br from-[#1c4e56] to-[#0f2a30] p-8 text-center text-white md:rounded-2xl md:p-12 lg:p-16">
          ...
          <button
            className="btn btn-primary md:btn-lg mt-5 justify-center px-6 py-2.5 text-sm md:mt-6"
            onClick={() => navigate({ to: '/register' })}
          >
            Create account →
          </button>
